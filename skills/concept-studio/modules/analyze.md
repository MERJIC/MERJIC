...
轻量索引：`/Users/myke/Documents/MERJIC/概念库/memory/concept_lite.json`
图结构索引：`/Users/myke/Documents/MERJIC/概念库/memory/concept_graph.json`
管理索引：`/Users/myke/Documents/MERJIC/概念库/memory/concept_meta.json`（可选）

---

## 索引优先原则

关联分析需要读取轻量索引 + 图结构索引两个文件（按需可选管理索引）。索引固定路径见文件头，找不到说明路径写错了，检查后重试。
新增/修改概念页后，运行 `python3 /Users/myke/Documents/MERJIC/概念库/scripts/build_index.py --incremental` 刷新索引。
...
**全量模式**（用户说「全量重跑」「重建基准」）
- 先运行 `python3 /Users/myke/Documents/MERJIC/概念库/scripts/build_index.py` 重建索引
...
读取 `/Users/myke/Documents/MERJIC/概念库/memory/concept_lite.json` 和 `/Users/myke/Documents/MERJIC/概念库/memory/concept_graph.json`。
需要文件路径/来源/日期时可选读取 `/Users/myke/Documents/MERJIC/概念库/memory/concept_meta.json`。
...
**任一分片找不到时**：检查路径是否正确，索引文件固定在 `/Users/myke/Documents/MERJIC/概念库/memory/` 下。
...
- 只分析 `[[]]` 链接，不分析语义相似度
- 集群归属建议需麦橘确认，不自动写入集群定义
- 断链只记录，不自动修复（修复由 `concept-ingest` 或麦橘手动处理）
- Apply 分析只报告，不硬写链接
